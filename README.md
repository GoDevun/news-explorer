...
| `MONGO_URL` | a MongoDB Atlas connection string, copied from Atlas rather than typed |
| `CORS_ORIGINS` | `https://godevun.github.io` |

`JWT_SECRET` is generated by Render. The health check path is `/status`, which
reports whether the provider key is configured and whether the database is
connected.

The API listens before the database is up and keeps retrying in the background,
so a database that is unreachable, misconfigured or still waking cannot take the
service down: the scored news feed needs no database and keeps serving, while
the account routes answer 503 until the connection succeeds.
